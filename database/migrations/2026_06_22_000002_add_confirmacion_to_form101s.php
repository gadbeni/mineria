...
class AddConfirmacionToForm101s extends Migration
{
    public function up()
    {
        Schema::table('form101s', function (Blueprint $table) {
            $table->tinyInteger('confirmado')->default(0)->after('observation');
            $table->timestamp('confirmed_at')->nullable()->after('confirmado');
    ...
    public function down()
    {
        Schema::table('form101s', function (Blueprint $table) {
            ...
            $table->dropColumn(['confirmado', 'confirmed_at', 'confirmed_by', 'signature_id']);
        });
    }
}
